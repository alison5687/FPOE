...
class Interfaz:
    def mostrar_interfaz(menu,modelo,validacion,comunicacion):
        ...
        def guardar():
            hilo=entry_hilo.get()
            elasticidad= modelo.elasticidad.get()
            suavidad= modelo.suavidad.get()
            tenacidad= modelo.tenacidad.get()
            ductilidad= modelo.ductilidad.get()

            resultado= comunicacion.guardar(hilo,elasticidad,suavidad,tenacidad,ductilidad)
            if resultado:
                showinfo("Exito","Hilo guardado.")
            else:
                showerror("Error","Error al guardar hilo.")
